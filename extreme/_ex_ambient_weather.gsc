...
lightningSky()
{
	...
	pos = getRandomPosPlayArea(400);
	if(level.ex_weather_thunder) thread lightningThunder(pos);

	switch(flash[flash_type])
	{
		case "single":
		{
			playfx(level.ex_effect["lightning"], pos);
			break;
		}
		case "double":
		{
