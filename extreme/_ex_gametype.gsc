...
init:
The init routine calls the initial setup for the gametype (sd, ctf, etc.)

getRespawnDelay:
Returns respawn delay due to various variables and conditions

getActivePlayers:
Returns the number of active players.

swapTeams(flagproc)
Swaps teams at halftime or every round.  flagproc is the return flag redirect, suitable only in ctf and ctfb, where there is a flag that returns or not

startOvertime(flagproc)   flagproc is the return flag redirect (see above)
Starts overtime
