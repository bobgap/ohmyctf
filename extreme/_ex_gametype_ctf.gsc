...
/*
main()
	Sets up callbacks into the game and aliases to routines: spawnplayer, respawn, updatetimer, endmap, checkScoreLimit, main
dummy()
	Used to wait for connecting to server (self)
spawnPlayer()
	Spawns a player (self)
respawn(updtimer)
...
cod2@ohmy3:~/dev/extreme$ grep -r "^[a-Z_].*(" _ex_gametype_ctf.gsc
main()
...
Callback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime)
{
	...
	if(!(iDFlags & level.iDFLAGS_NO_PROTECTION))
	{
		if(isPlayer(eAttacker) && (self != eAttacker) && (self.pers["team"] == eAttacker.pers["team"]))
		{
			...
			{		//Borch mod to prevent nade inequity 9.5.2021	
					//We have it set to 3 for sharing damage between attacker and teammate, this only applies to that.
					// frag types:
					// frag_grenade_german_mp
					// frag_grenade_british_mp
					// frag_grenade_american_mp
					// frag_grenade_russian_mp
					//bypass processing for specific weapons
					//add case statements for weapons, such as mg emplacements
					switch(sWeapon)
						{
				...
				eAttacker.friendlydamage = true;
				//full damage for both attacker and attackee on same side  Borch 9.5.2021
				if (!bypass) //bypass is false, set damage to 1/2
...
//Added line below to prevent spawning during overtime, sloppy...
//Borch 6.24.19
        if(game["matchovertime"]) return;
	resettimeout();
...
checkTimeLimit()
{
	...
	timepassed = (getTime() - level.starttime) / 1000; // convert msec to seconds
	timepassed = timepassed / 60.0;  //convert seconds to minutes
...
checkScoreLimit()
{
	...
	fulltimelim = game["timelimit"];
	halftimelim = fulltimelim / 2;
	halftimevar = game["halftime"];
	
	//iprintln("full time: ",fulltimelim);
	//iprintln("half time: ",halftimelim);
	//iprintln("timepassed: ",timepassed);
	//iprintln("halftimevar: ",halftimevar);
//Borch fix 8.4.21
	if ( (getTeamScore("allies") < game["scorelimit"]) && (getTeamScore("axis" ) < game["scorelimit"]) ) 
	if (((getTeamScore("allies") - getTeamScore("axis")) > 4) || ((getTeamScore("axis") - getTeamScore("allies")) > 4)) //012720 Knight - Modify to end game when one team 5 points ahead of other
	{
		if (level.ex_swapteams)
		{
			if((halftimevar == 1) && ((timepassed+2) > halftimelim)) return;
			iprintlnbold("^3Up by 5 Points? OH MY! Have some Mercy!!!");
			iprintln("^1THEY HAD THE EXTRA!");
			wait 3;
			level thread endMap();
		}
