	...
	while(isAlive(self) && self.sessionstate=="playing" && myteam == self.pers["team"])
	{
		...
		if(isDefined(level.flag) && !level.flag.stolen)
		{
			...
			if(self isTouchingFlag() && !level.flag.stolen)
			{
				...
				// Get personal score
				self thread [[level.ex_scorePlayer]](level.PointsForStealingFlag);
			}
		}
			...
			if(isDefined(self.flag))
			{
				...
				if(count >= 20)
				{
					...
					if(level.holdtime >= level.flagholdtime)
					{
						...
						self.pers["flagcap"]++;
						if(level.ex_statshud) self thread extreme\_ex_stats_hud::showStatsHUD();
						...
						self thread [[level.ex_scorePlayer]](level.PointsForHoldingFlag);
					}
