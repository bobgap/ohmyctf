...
endRound(roundwinner)
{
	...
	if(level.ex_swapteams) extreme\_ex_gametype::swapTeams();
	else
	{
		...
		if(isPlayer(other) && isAlive(other) && (other.pers["team"] != "spectator") && !level.roundended)
		{
			...
			if(other.pers["team"] == self.team) // Touched by team
			{
				if(self.atbase)
				{
					if(isDefined(other.flag)) // Captured flag
					{
						...
						other.pers["flagcap"]++;
						other thread [[level.ex_scorePlayer]](level.ex_rbctfpoints_playercf, "special");
				...
				else // Returned flag
				{
					...
					other.pers["flagret"]++;
					other thread [[level.ex_scorePlayer]](level.ex_rbctfpoints_playerrf, "special");
				}
			}
			else if(other.pers["team"] != self.team) // Touched by enemy
			{
				...
				if(self.atbase) // Stolen flag
				{
					...
					other thread [[level.ex_scorePlayer]](level.ex_rbctfpoints_playersf, "special");
				}
				else // Picked up flag
				{
					lpselfnum = other getEntityNumber();
					...
					if(!isDefined(other.flagDropped))
						other thread [[level.ex_scorePlayer]](level.ex_rbctfpoints_playertf, "special");
				}
...
isnearCarrier(attacker)
{
	...
	if(isDefined(attacker.flag)) return(false);
	...
	for(i = 0; i < players.size; i++)
	{
		...
		if(!isDefined(player.flag)) continue;
		if(player.pers["team"] != attacker.pers["team"]) continue;
