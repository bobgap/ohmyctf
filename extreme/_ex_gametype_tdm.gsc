...
checkTeamStatus()
{
	...
	if(!level.exist["allies"])
	{
		...
		alliedscore = getTeamScore("allies");
		axisscore = getTeamScore("axis");
		if(alliedscore >= axisscore) thread [[level.ex_scoreTeam]]("axis", alliedscore + 1);
	...
	if(!level.exist["axis"])
	{
		...
		alliedscore = getTeamScore("allies");
		axisscore = getTeamScore("axis");
		if(axisscore >= alliedscore) thread [[level.ex_scoreTeam]]("allies", axisscore + 1);
...
checkTimeLimit()
{
	...
	if(level.ex_swapteams && !game["matchovertime"])
	{
		...
		if(game["halftime"] == 0)
		{
			thread extreme\_ex_gametype::swapTeams();
			return;
		}
	}
	...
	if(level.ex_overtime && !game["matchovertime"])
	{
		...
		{
			thread extreme\_ex_gametype::startOvertime();
			return;
		}
	}
