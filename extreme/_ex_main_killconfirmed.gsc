...
dogtagPickupMonitor(index)
{
	...
	while(true)
	{
		...
		if( (!level.ex_teamplay && player == kc_record.victim) || (level.ex_teamplay && player.pers["team"] == kc_record.victim_team) )
		{
			...
			{
				player thread [[level.ex_scorePlayer]](level.ex_kc_denied_bonus, "bonus");
				if(kc_record.isteamscore) thread [[level.ex_scoreTeam]](player.pers["team"], level.ex_kc_denied_bonus);
			}
		...
		else
		{
			...
			pointsto thread [[level.ex_scorePlayer]](points, "bonus", kc_record.reward);
			if(kc_record.isteamscore) thread [[level.ex_scoreTeam]](pointsto.pers["team"], points);
...
dogtagConfirmHUD(notification)
{
	...
	playerHudFade(hud_index, 0.5, 0, 0);
