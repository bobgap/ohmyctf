...
main()
{
	if(!level.ex_stbd) iprintlnboldCLEAR("all", 5);
...
PrepareMapVote()
{
	...
	for(i = 0; i < players.size; i++)
	{
		...
		// No voting for non-clan spectators (unless there is only one player)
		if(level.players.size > 1)
		{
			if(isDefined(player.pers["team"]) && player.pers["team"] == "spectator" || player.sessionteam == "spectator")
				if(!player.ex_clanID) player.mv_allowvote = false;
		}

		// No voting for testclients (bots)
		if(isDefined(player.pers["isbot"])) player.mv_allowvote = false;
		...
		if(level.ex_clanvoting && !level.ex_mapvoteignclan)
			if(!player.ex_clanID || !level.ex_clanvote[player.ex_clanID])
	...
	if(level.ex_mapvotemode < 4)
	{
		...
		{
			case 1: { mv_maprot = extreme\_ex_main_rotation::getRandomMapRotation(); break; }
			case 2: { mv_maprot = extreme\_ex_main_rotation::getPlayerBasedMapRotation(); break; }
			case 3: { mv_maprot = extreme\_ex_main_rotation::getRandomPlayerBasedMapRotation(); break; }
			default: { mv_maprot = extreme\_ex_main_rotation::getPlainMapRotation(); break; }
		}
		...
		if(level.ex_mapvotereplay == 2)
		{
			...
			level.mv_items[0]["gametypename"] = extreme\_ex_main_maps::getgtstringshort(level.ex_currentgt);
		...
		for(j = 0; j < mv_maprot.size; j++)
		{
			...
			{
				if(level.ex_log_mapvote) logprint("MPV: omitting map " + mv_maprot[j]["map"] + ". Map has just been played\n");
				continue;
			}
			...
			{
				if(level.ex_log_mapvote) logprint("MPV: omitting map " + mv_maprot[j]["map"] + ". Map has been played recently (memory)\n");
				continue;
			}

			// Fill the candidate entry
			level.mv_items[i]["map"] = mv_maprot[j]["map"];
			level.mv_items[i]["mapname"] = extreme\_ex_main_maps::getmapstring(mv_maprot[j]["map"]);
			level.mv_items[i]["gametype"] = mv_maprot[j]["gametype"];
			level.mv_items[i]["gametypename"] = extreme\_ex_main_maps::getgtstringshort(mv_maprot[j]["gametype"]);
		...
		if(level.ex_mapvotereplay == 1)
		{
			...
			level.mv_items[replayentry]["gametypename"] = extreme\_ex_main_maps::getgtstringshort(level.ex_currentgt);
	...
	else
	{
		...
		if(level.ex_log_mapvote) logprint("MPV: number of players for mapvote = " + mv_numplayers + " (out of " + level.players.size + " players)\n");
		mv_skipmemcheck = false;
		mv_currentmapix = 0;

		for(run = 1; run <= 3; run++)
		{
			if(level.ex_log_mapvote) logprint("MPV: map selection run " + run + "\n");
			level.mv_items = [];
			...
			for(j = 0; j < mv_maplist.size; j++)
			{
				...
				{
					if(level.ex_log_mapvote) logprint("MPV: omitting map " + mv_maplist[j]["map"] + ". Map has just been played\n");
					mv_currentmapix = j;
					if(level.ex_mapvotereplay == 2)
					{
				...
				{
					if(level.ex_log_mapvote) logprint("MPV: omitting map " + mv_maplist[j]["map"] + ". Map has been played recently (memory)\n");
					continue;
				}
				...
				if(level.ex_mapvote_filter)
				{
					if(level.ex_mapvote_filter == 2)
					{
						if(mv_maplist[j]["playsize"] == level.ex_mapsizing_large) // Large map
						{
							...
							{
								if(level.ex_log_mapvote) logprint("MPV: omitting large map " + mv_maplist[j]["map"] + ". There are " + mv_numplayers + " players. Map requires at least " + mv_maplist[j]["playsize"] + " players\n");
								continue;
							}
						}
						else if(mv_maplist[j]["playsize"] == level.ex_mapsizing_medium) // Medium map
						{
							...
							{
								if(level.ex_log_mapvote) logprint("MPV: omitting medium map " + mv_maplist[j]["map"] + ". There are " + mv_numplayers + " players. Map requires between " + mv_maplist[j]["playsize"] + " and " + (level.ex_mapsizing_large - 1) + " players\n");
								continue;
							}
						}
						else if((mv_maplist[j]["playsize"] == 1) && (mv_numplayers >= level.ex_mapsizing_medium)) // Small map
						{
							if(level.ex_log_mapvote) logprint("MPV: omitting small map " + mv_maplist[j]["map"] + ". There are " + mv_numplayers + " players. Map requires less than " + level.ex_mapsizing_medium + " players\n");
							continue;
						}
					}
					else if(mv_numplayers < mv_maplist[j]["playsize"])
					{
						if(level.ex_log_mapvote) logprint("MPV: omitting map " + mv_maplist[j]["map"] + ". There are " + mv_numplayers + " players. Map requires " + mv_maplist[j]["playsize"] + " players\n");
						continue;
					}
				}
...
VoteLogicRotation()
{
	...
	gametypename = extreme\_ex_main_maps::getgtstringshort(gametype); // only short strings are precached!
...
VoteLogicList()
{
	...
	for(j = 0; j < gt_array.size; j++)
	{
		...
		if(gt_allowed)
		{
			...
			level.mv_items[gt_index]["gametypename"] = extreme\_ex_main_maps::getgtstring(gt_array[j]);
			level.mv_items[gt_index]["votes"] = 0;
			gt_index++;
		}
		else if(level.ex_log_mapvote) logprint("MPV: Ignoring game type " + gt_array[j] + ". Disabled in mapcontrol.cfg (see ex_endgame_vote_allow_" + gt_array[j] + ")\n");
	}
	...
	if(!isDefined(level.mv_items))
	{
		...
		level.mv_items[gt_index]["gametypename"] = extreme\_ex_main_maps::getgtstring("tdm");
		level.mv_items[gt_index]["votes"] = 0;
	}
	...
	if(level.ex_mapvoteweaponmode)
	{
		...
		if(level.mv_itemsmax > 1)
		{
			...
			//levelHudSetText("mapvote_title", extreme\_ex_main_maps::getgtstringshort(gametype));
...
mapvoteMemory(mapname, maxmaps)
{
	...
	if(level.ex_log_mapvote)
	{
		...
		logprint("MPV: maps in memory, including last map played. The | character marks the max for the current rotation:\n");
		logprint("MPV: memory [ " + maps_in_memory + "]\n");
	}
}
...
strCompare(str1, str2)
{
	...
	monostr1 = tolower( monoString(str1.longname) );
	monostr2 = tolower( monoString(str2.longname) );
...
{
	for(i = 1; i < level.ex_maps.size; i++)
		logprint("MPV: Map " + i + ": " + monoString(level.ex_maps[i].longname) + " (" + level.ex_maps[i].mapname + ")\n");
}
