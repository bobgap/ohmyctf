#include extreme\_ex_player_punish;
	...
	for(i = 0; i < players.size; i++)
	{
		...
		if(player.sessionstate == "playing" && ((entID == playerEntID) || (playerEntID == -1) || (v_team != "" && player.pers["team"] == v_team)) )
		{
			if(isAlive(player))
			{
				...
				{
					if(!isDefined(player.pers["savedmodel"])) player extreme\_ex_main_models::getModel();
						else extreme\_ex_main_models::loadModel(player.pers["savedmodel"]);
			...
			if(side != player.pers["team"])
			{
				...
				switch(side)
				{
					case "spectator":
						...
						player extreme\_ex_main_clientcontrol::clearWeapons();
					...
					case "axis":
						...
						if(isDefined(player.pers["isbot"])) player thread extreme\_ex_main_bots::dbotJoin(side);
