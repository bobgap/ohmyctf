						...
						else
						{
							...
							if(!level.ex_bash_only)
							{
								if((level.ex_arcade_shaders & 16) == 16) self thread extreme\_ex_player_arcade::showArcadeShader("x2_humiliation", level.ex_arcade_shaders_extra);
									else if(self.ex_obmonpmsg) self iprintln(&"OBITUARY_HUMILIATION");
			...
			if(level.ex_firstblood && !level.ex_firstblood_done)
			{
				...
				for(i = 0; i < players.size; i++)
				{
					if(players[i] != self)
					{
						...
						players[i] iprintln(&"OBITUARY_FIRSTBLOOD_ALL", [[level.ex_pname]](attacker));
						if(!level.ex_teamplay || attacker.pers["team"] != self.pers["team"]) players[i] iprintln(&"OBITUARY_FIRSTBLOOD_VICTIM", [[level.ex_pname]](self));
				...
				self iprintln(&"OBITUARY_FIRSTBLOOD_SELF");
