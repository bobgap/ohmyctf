...
rconMain(command)
{
	...
	switch(command)
	{
		...
		case "rcon_cmd_pin":
		{
			//if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " sent PIN number " + rcon_pin_entry + "\n");
		...
		case "rcon_cmd_pinenter":
		{
			...
			{
				if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " submitted PIN \"" + self.ex_rcon_pin_entry +"\" for validation\n");
				if(self.ex_rcon_pin_entry == self.ex_rcon_pin)
				{
					self playlocalsound("access_granted");
					if(level.ex_log_rcon) logprint("RCON: Player " + self.name + ": AUTHORIZED\n");
					rconAuthorizeLogin();
				...
				else
				{
					...
					if(level.ex_log_rcon) logprint("RCON: Player " + self.name + ": INVALID PIN\n");
					rconLoginFalsePIN();
				}
			}
		...
		{
			//if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " cleared PIN\n");
			self.ex_rcon_pin_entry = "";
			break;
		}
		case "rcon_cmd_login":
		{
			if(level.ex_log_rcon) logprint("RCON: Player " + self.name + ": LOGGED IN\n");
			rconAuthorizeAccess();
		...
		if(rcon_action != "")
		{
			switch(rcon_paramode)
			{
					...
					switch(self.ex_rcon_action)
					{
						case 1:
							if(self.ex_rcon_map != 999 && isDefined(level.ex_maps[self.ex_rcon_map].mapname))
							{
								...
								if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " changing map to " + rcon_map + "\n");
						...
						case 11:
							if(self.ex_rcon_gametype != 0)
							{
								...
								if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " changing game type to " + rcon_gt + "\n");
				...
				case 2:
				{
					...
					{
						case 2:
							if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " ending current map\n");
						...
						case 6:
							...
							{
								rcon_map = level.ex_maps[self.ex_rcon_map].mapname;
								if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " changing next map to " + rcon_map + "\n");
								self setClientCvar("ui_rconExtremeCmd", "rcon " + rcon_action + " " + rcon_map);
								self setClientCvar("rcon_rsp_apply", 1);
							}
							break;
						case 7:
							if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " skipping next map\n");
							self setClientCvar("ui_rconExtremeCmd", "rcon " + rcon_action + " " + self.ex_rcon_action);
							self setClientCvar("rcon_rsp_apply", 1);
							break;
						default:
				...
				default:
				{
					if(rcon_action != "set exrcon" && level.ex_log_rcon) logprint("RCON: Player " + self.name + " doing a " + rcon_action + "\n");
...
rconMapServerAction(action, map, gametype)
{
	...
	//if(level.ex_log_rcon) logprint("RCON: Action " + action + " executed by server\n");
...
{
;
//disabled by Borch (hopefully) 3.27.20
	...
	if(self.ex_rcon_action != -1)
	{
		switch(self.ex_rcon_action)
		{
			...
			case 5:
				...
				if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " activated profile " + self.ex_rcon_action + ", \"" + game["drm_profiles"][self.ex_rcon_action].name + "\" for the next map\n");
				break;
...
rconInitAuthorization(authno)
{
	...
	else
	{
		...
		if(self.ex_rcon_pin == "")
		{
			...
			if(level.ex_log_rcon) logprint("RCON: No PIN defined for player " + self.name + ". Assigned random PIN " + self.ex_rcon_pin + "\n");
		}
		self.ex_rcon_pin_entry = "";
...
rconIsAuthorizedFor(menu)
{
	...
	if(!rcon_authok)
	{
		...
		logprint("RCON: Player " + self.name + " kicked for unauthorized access attempt\n");
		rconResetLoginInfo();
...
rconLoginTimeframe(seconds)
{
	...
	if(!rcon_loggedin)
	{
		if(level.ex_log_rcon) logprint("RCON: Player " + self.name + " missed the window of opportunity to log in (" + seconds + " seconds)\n");
		self.ex_rcon_authorized = 0;
		self setClientCvar("ui_rconExtremeAccess0", 0);
		self setClientCvar("ui_rconExtremeLogin", "set exrcon 1");
	}
...
rconLoginFalsePIN()
{
	...
	{
		logprint("RCON: Player " + self.name + " kicked for exceeding allowed number of login attempts\n");
		kick(self getEntityNumber());
	}
}
