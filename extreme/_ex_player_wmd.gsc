...
{	
	...
	// game["halftime"] means we passed halftime if 1, didn't if 0
	// level.ex_swapteams is 2 if we have a halftime match
	// only prevent airstrike late on fulltime map or halftime map on second half, indicated by game["halftime"]==1
	//since this is only called every six seconds, prevent calling wmd at 21 seconds before end of game
	// calculate timepassed	

	timepassed = (getTime() - level.starttime) / 1000; // convert msec to seconds
	timelimitsecs=game["timelimit"] * 60;
	//check for swapteams first to prevent undefined error for game("halftime")
	
	can_halt=0;  //
	if(level.ex_swapteams==2)
			{
			timelimitsecs=timelimitsecs/2;
			if(game["halftime"]==1)
				can_halt=1; //means this time bit will lead to overtime
			}
		else //fulltime map
			can_halt=1;
	remaining=timelimitsecs-timepassed;			
	
	// logprint("remaining=" + remaining + ", timelimitsecs=" + timelimitsecs + ", timepassed=" + timepassed + "\n");
	// if playing LIB and player is jailed, do not give WMD
	//added "|| remaining<22 and can_halt==1" below bobgap 11/2021
	if(level.ex_currentgt == "lib" && isDefined(self.in_jail) && self.in_jail || (remaining<22 && can_halt==1))
		{
			self wmdStop();
			return;
		}
