...
startWmd(delay, first, next, napalm)
{
	...
	self notify("end_airstrike");//kill previous airstrike
	wait( [[level.ex_fpstime]](0.1) ); //wait to initiate next airstrike
	self endon("end_airstrike"); //restore end state of airstrike
	...
	while(self.ex_airstrike)
	{
		...
		if(!self.ex_napalm)
		{
			if((level.ex_arcade_shaders & 4) == 4) self thread extreme\_ex_player_arcade::showArcadeShader("x2_airstrikeunlock", level.ex_arcade_shaders_perk);
				else self iprintlnbold(&"AIRSTRIKE_READY");	//shaders put up icons instead of text
		}
		else
		{
		...
		// now wait for one "next" length interval between airstrikes (fpstime adjusted to frame rate, 30 or 20, multiplied by next.)
