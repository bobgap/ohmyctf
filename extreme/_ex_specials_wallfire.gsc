...
perkAssign(index, delay)
{
	...
	if(!level.ex_store_keeptimer) self thread playerStartUsingPerk(index, false);
...
perkThink(index)
{
	...
	while(timer < level.ex_wallfire_timer)
	{
		...
		timer++;
		if(timer == level.ex_store_keeptimer) self thread playerStartUsingPerk(index, false);
	}
...
logWeaponPenetration(weapon, dist, wall, surface)
{
	...
	logprint("SPC: [wallfire] Weapon " + weapon + ": strength[" + strength + "] * ((range[" + range + "] - dist[" + dist + "]) / range[" + range + "]) = " + strength1 + "\n");
	if(level.ex_wallfire_friction)
	{
		friction = getSurfaceFriction(surface);
		strength = int(strength1 - (wall * friction));
		logprint("SPC: [wallfire] Surface strength[" + strength1 + "] - (wall[" + wall + "] * friction[" + friction + "]) = " + strength + "\n");
	}
	else
	{
		strength = int(strength1 - wall);
		logprint("SPC: [wallfire] Surface strength[" + strength1 + "] - wall[" + wall + "] = " + strength + "\n");
	}
	return(strength);
}
...
initWeaponArray()
{
	...
	for(i = 0; i < level.weaponnames.size; i++)
	{
		...
		//logprint("SPC: [wallfire] Registered weapon " + weaponname + "\n");
		...
		if(weapon_checklr)
		{
			...
			if(counterpart != "none")
			{
				...
				//logprint("SPC: [wallfire] Registered LR weapon " + counterpart + "\n");
			}
		}
	}
}
...
{
	effect_id = extreme\_ex_main_utils::isInEffectsArray(game["precached_effects"], effect);
	if(effect_id == -1)
	{
