...
init()
{
	...
	if(level.ex_teamplay)
	{
		...
		else
		{
			for(;;)
			{
				//added by Borch 5.6.20 to eliminate overtime balancing, still puts up the notice though (11.2021)
				//added:  && !game["matchovertime"] to the line below
				if(level.teambalance && !game["matchovertime"])
				{
					if(level.ex_teambalance_delay)
					{
						wait( [[level.ex_fpstime]](level.ex_teambalance_delay) );
					...
					secondsleft = 999;
					if(game["timelimit"]) // is there time left in the game?
					{
						passedtime = (getTime() - level.starttime) / 1000;   //calculate seconds used in game so far
						if(level.ex_overtime && game["matchovertime"])  //what is this??  This whole portion should never activate.
								secondsleft = int( (game["timelimit"] * 60) - passedtime + 0.5 );
