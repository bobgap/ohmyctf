...
init()
{
	...
	for(i = 0; i < level.weaponnames.size; i++)
	{
		...
		{
			weaponspec = extreme\_ex_main_utils::trim( [[level.ex_drm]](weaponname, "", "", "", "string") );
			...
			level.weapons[weaponname].allow = strToIntMinMax(weaponspec_array[0], 1, 0, 1);
			level.weapons[weaponname].limit = strToIntMinMax(weaponspec_array[1], 0, 0, 128);
			level.weapons[weaponname].clip_limit = strToIntMinMax(weaponspec_array[2], 999, 0, 999);
			level.weapons[weaponname].ammo_limit = strToIntMinMax(weaponspec_array[3], 999, 0, 999);
			level.weapons[weaponname].wdm = strToIntMinMax(weaponspec_array[4], 100, 0, 1000);
			...
			//logprint("WPN: " + weaponname + ": " + level.weapons[weaponname].allow + "," + level.weapons[weaponname].limit + "," + level.weapons[weaponname].clip_limit + "," + level.weapons[weaponname].ammo_limit + "," + level.weapons[weaponname].wdm + "\n");
		}
	}
	...
	if(level.ex_wdmodon)
	{
		for(i = 0; i < level.weaponnames.size; i++)
		{
			...
			{
				level.weapons[weaponname].wdm = [[level.ex_drm]]("ex_wdm_" + weaponname, 100, 0, 1000, "int");
				//logprint("WPN: " + weaponname + ": " + level.weapons[weaponname].wdm + "\n");
			}
		}
	}
	...
	for(i = 0; i < level.weaponnames.size; i++)
	{
		...
		if(isDefined(level.weapons[weaponname].parent))
		{
			...
			{
				logprint("WPN: Weapon " + weaponname + " in weapons array references a non existing parent!\n");
...
deleteRestrictedWeapons()
{
	...
	for(i = 0; i < level.weaponnames.size; i++)
	{
		...
		if(level.weapons[weaponname].classname == "frag")
		{
			...
			{
				extreme\_ex_monitor_entities::removeEntity("weapon_frag_grenade_american_mp", "classname");
				extreme\_ex_monitor_entities::removeEntity("weapon_frag_grenade_british_mp", "classname");
				extreme\_ex_monitor_entities::removeEntity("weapon_frag_grenade_russian_mp", "classname");
				extreme\_ex_monitor_entities::removeEntity("weapon_frag_grenade_german_mp", "classname");
			}
		}
		else if(level.weapons[weaponname].classname == "smoke")
		{
			if(!level.weapons[weaponname].allow)
			{
				extreme\_ex_monitor_entities::removeEntity("weapon_smoke_grenade_american_mp", "classname");
				extreme\_ex_monitor_entities::removeEntity("weapon_smoke_grenade_british_mp", "classname");
				extreme\_ex_monitor_entities::removeEntity("weapon_smoke_grenade_russian_mp", "classname");
				extreme\_ex_monitor_entities::removeEntity("weapon_smoke_grenade_german_mp", "classname");
			}
		}
		else
		{
			if(level.ex_bash_only || !level.weapons[weaponname].allow || !level.weapons[weaponname].precached)
				extreme\_ex_monitor_entities::removeEntity("weapon_" + weaponname, "classname");
		}
	}	
	...
	if(level.ex_modern_weapons)
	{
		for(i = 0; i < level.oldweaponnames.size; i++)
			extreme\_ex_monitor_entities::removeEntity("weapon_" + level.oldweaponnames[i], "classname");
	}
}
...
dropWeapon()
{
	...
	if(level.ex_sprint && level.ex_wepo_drop_sp && extreme\_ex_main_utils::tooClose(100, false, false, false, false)) return;
...
dropOffhand()
{
	...
	{	
		...
		//Borch, 5.4.20: restrict dropped nades to one of your own kind.
		if (fragsize + enemy_fragsize > 1)
			{
			//onlydrop own frags, maximum of one
			fragsize = 1;
			enemy_fragsize = 0;
			self setWeaponClipAmmo(self.pers["fragtype"], 1);//Borch 5.5.2020 set frags to qty of 1
			}	
	//end of Borch 5.4.20 addition			
	}
	...
	{
	// Borch added randomint test to following line 5.4.20
	// set the number after the <= to the integer from 1 to 100
	// the higher the number the greater the chance (percentage)
		if( fragsize && randomint(100) <=30 ) 
			self dropItemIfAllowed(self.pers["fragtype"]);
	//  Borch disabled dropping of enemy nade, 5.4.20
	...
	if(level.ex_store && level.ex_supernade && level.ex_supernade_drop)
	{
...
{
	// do not drop mobile MG's
	//if(isWeaponType(weapon, "mobilemg")) return;
...
getFragGrenadeCount()
{
	...
	//Borch 5.4.20 restricted nade quantity to 1
	if (count > 1) count = 1;
	return(count);
}
...
{
	if(self.pers["team"] == "allies") grenadetype = getSmokeTypeAllies();
		else grenadetype = getSmokeTypeAxis();
			...
			if(level.weapons[response].limit > 0)
			{
				if(level.ex_teamplay && level.ex_wepo_limiter_perteam)
				{
					if(self.pers["team"] == "allies")
					{
						...
						else logprint("WPN: level.weapons[" + response + "].allow_allies does not exist\n");
					}
					else
					{
						...
						else logprint("WPN: level.weapons[" + response + "].allow_axis does not exist\n");
					}
				}
				else
				{
					...
					else logprint("WPN: level.weapons[" + response + "].allow does not exist\n");
				}
			}
		}
		else logprint("WPN: level.weapons[" + response + "] does not exist\n");
	}
...
updateAllowed()
{
	...
	for(i = 0; i < level.weaponnames.size; i++)
	{
		...
		//logprint("WPN: Checking allow status for weapon " + weaponname + " (new " + cvarvalue + " vs old " + level.weapons[weaponname].allow + ")\n");
...
registerWeapon(weaponname, classname, subclass, status, team, nationality, locstr, usean, parent)
{
	...
	level.weapons[weaponname].usean = usean;
...
registerWeaponsOther()
{
	...
	// tripwires
	if(level.ex_tripwire) registerWeapon("tripwire_mp", "tripwire", undefined, 0, "all", 15, &"WEAPON_TRIPWIRE", 0);
	...
	if(level.ex_gunship || ((level.ex_store & 2) == 2 && level.ex_gunship_special))
	{
	...
	// WMD
	if(level.ex_wmd || level.ex_mortars == 2) registerWeapon("mortar_mp", "wmd", undefined, 0, "all", 15, &"WEAPON_MORTAR", 0);
	if(level.ex_wmd || level.ex_artillery == 2 || level.ex_cmdmonitor) registerWeapon("artillery_mp", "wmd", undefined, 0, "all", 15, &"WEAPON_ARTILLERY", 1);
	if(level.ex_wmd || level.ex_planes == 3) registerWeapon("planebomb_mp", "wmd", undefined, 0, "all", 15, &"WEAPON_AIRSTRIKE", 0);
...
precacheWeapons()
{
	...
	{
		[[level.ex_PrecacheItem]]( getFragTypeAxis() );
		[[level.ex_PrecacheItem]]( getFragTypeAllies() );
	...
	{
		[[level.ex_PrecacheItem]]( getSmokeTypeAxis() );
		[[level.ex_PrecacheItem]]( getSmokeTypeAllies() );
	...
	// dummy placebo weapons
	[[level.ex_PrecacheItem]]("dummy1_mp");
	[[level.ex_PrecacheItem]]("dummy2_mp");
	[[level.ex_PrecacheItem]]("dummy3_mp");

	// mortar placebo weapon
	if(level.ex_wmd || level.ex_mortars) [[level.ex_PrecacheItem]]("mortar_mp");

	// artillery placebo weapon
	if(level.ex_wmd || level.ex_artillery || level.ex_cmdmonitor) [[level.ex_PrecacheItem]]("artillery_mp");

	// airstrike placebo weapons
	if(level.ex_wmd || level.ex_planes) [[level.ex_PrecacheItem]]("planebomb_mp");

	// landmine placebo weapon
	if(level.ex_landmines) [[level.ex_PrecacheItem]]("landmine_mp");

	// tripwire placebo weapon
	if(level.ex_tripwire) [[level.ex_PrecacheItem]]("tripwire_mp");
...
bridgePrecacheItem(weaponname)
{
	...
	if(precache)
	{
		...
		{
			counterpart = extreme\_ex_weapons_longrange::getWeaponCounterpart(weaponname, false);
...
getMBotWeapon(weapon)
{
	...
	// if unknown weapon, return dummy weapon. Dummies are always precached anyway,
	// and the botJoin code will automatically skip it
	return("dummy1_mp");
}
